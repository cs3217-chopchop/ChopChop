...
class RecipeStepParserTests: XCTestCase {
    ...
    func testParseTimeTaken() {
        let timeTaken = RecipeStepParser.parseTotalDuration(step: """
...
""")
 ...
 """)
    ...
    func testParseTimeTaken_noTimeMention() {
        let timeTaken = RecipeStepParser.parseTotalDuration(step: "Preheat an oven to 425 degrees F (220 degrees C).")
    ...
    func testParseTimeDurations() {
        let durations = RecipeStepParser.parseTimeStrings(step: """
...
""")
    ...
    func testParseTimeDurations_fraction() {
        let durations = RecipeStepParser.parseTimeStrings(step: "set timer for 2 1/3 h")
    ...
    func testParseToTime_decimal() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "2.5 minutes")
        XCTAssertEqual(timeTaken, 150)
    }

    func testParseToTime_fraction() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "1 1/2 hours")
        XCTAssertEqual(timeTaken, 5_400)
    }

    func testParseToTime_unrecognisedFraction() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "½ m")
        XCTAssertEqual(timeTaken, 0)
    }

    func testParseToTime_complexfraction() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "2 and a half minute")
        XCTAssertEqual(timeTaken, 150)
    }

    func testParseToTime_withRangeNoSpace() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "20-25mins")
        XCTAssertEqual(timeTaken, 1_350)
    }

    func testParseToTime_wordBetween() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "15 more minutes")
        XCTAssertEqual(timeTaken, 900)
    }

    func testParseToTime_twoUnits() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "1 hour 10 minutes")
        XCTAssertEqual(timeTaken, 4_200)
    }

    func testParseToTime_twoUnitsWithRange() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "1h 20 min to 1h 30 min")
        XCTAssertEqual(timeTaken, 5_100)
    }

    func testParseToTime_numberWord() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "five to 10 minutes")
        XCTAssertEqual(timeTaken, 450)
    }

    func testParseToTime_negativeNumber_takeFirst() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "5h -40min")
        XCTAssertEqual(timeTaken, 18_000)
    }

    func testParseToTime_noUnits() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "500")
        XCTAssertEqual(timeTaken, 0)
    }

    func testParseToTime_differentUnits() {
        let timeTaken = RecipeStepParser.parseDuration(timeString: "30m to 1h 2 min")
